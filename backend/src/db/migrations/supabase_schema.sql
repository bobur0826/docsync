-- ============================================================
--  DocSync — Full Schema for Supabase
...
CREATE EXTENSION IF NOT EXISTS "vector";  -- pgvector (enable in Supabase Extensions first)
...
CREATE TABLE users (
  ...
  role          TEXT        NOT NULL DEFAULT 'user'
                CHECK (role IN ('admin','manager','engineer','user')),
...
CREATE TABLE documents (
  ...
  status          TEXT        NOT NULL DEFAULT 'draft'
                  CHECK (status IN ('draft','in_review','approved','rejected',
                                    'superseded','for_construction','for_information')),
...
CREATE TABLE transmittals (
  ...
  purpose         TEXT        NOT NULL CHECK (purpose IN ('for_review','for_construction',
...
CREATE TRIGGER trg_transmittals_updated_at
  BEFORE UPDATE ON transmittals FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TRIGGER trg_transmittal_items_updated_at
  BEFORE UPDATE ON transmittal_items FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TRIGGER trg_workflow_steps_updated_at
  BEFORE UPDATE ON workflow_steps FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE page_permissions (
  ...
  role      TEXT        CHECK (role IN ('admin','manager','engineer','user')),
...
CREATE INDEX idx_page_perms_tenant ON page_permissions(tenant_id);
